...
#' 
#' imageDimnames(volcano, axisLas = 2)
#' 
#' imageDimnames(provideDimnames(volcano))
#' 
#' tmp <- tapply(warpbreaks$breaks, warpbreaks[,-1], sum)
#' imageDimnames(tmp, drawBorders = TRUE, addLabels = TRUE, labelCol = 4)
#' 
imageDimnames <- function(mat, 
  xlab = NULL, ylab = NULL, xaxisSide = 1, yaxisSide = 2,
  axisLas = 1,
  drawBorders = FALSE, borderCol = 1, borderLty = 1, borderLwd = 1,
  addLabels = FALSE, labels = sinkr::ac(c(mat)), labelCol = "black", labelFont = 1,
  ...){
